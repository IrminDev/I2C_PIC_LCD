...
static char sendBuf[14 + 1];
static char recvBuf[14 + 1];
...
static char txBuf[14 + 1];
static uint8_t txLen;
static uint8_t txIdx;


static char rxTmp[14 + 1];
...
static volatile uint8_t refreshFlag;
...
static void lcd_init(void) {
    ...
    lcd_send_nibble(0x03); _delay((unsigned long)((150)*(4000000UL/4000000.0)));
    lcd_send_nibble(0x03); _delay((unsigned long)((50)*(4000000UL/4000000.0)));
    lcd_send_nibble(0x02); _delay((unsigned long)((50)*(4000000UL/4000000.0)));
...
static void buzzer_beep(uint16_t duration_ms) {
    PORTCbits.RC0 = 1;
    while (duration_ms--) _delay((unsigned long)((1)*(4000000UL/4000.0)));
    PORTCbits.RC0 = 0;
}
    ...
    if (!SSPSTATbits.R_nW) {
        if (!SSPSTATbits.D_nA) {
            ...
            if (rxState == 0) {
                ...
                rxExpected = data;
                if (rxExpected > 14) rxExpected = 14;
                rxIdx = 0;
                rxState = 1;
            } else {
                ...
                if (rxIdx >= rxExpected) {
                    ...
                    rxTmp[rxIdx] = '\0';
                    memcpy((void*)recvBuf, rxTmp, rxIdx + 1);
                    newMsgFlag = 1;
                    rxState = 0;
                }
            }
...
static void process_key(uint8_t key) {
    ...
    if (key == 14) {
        ...
        if (sendLen > 0) {
            ...
            PIE1bits.SSPIE = 0;
            memcpy(txBuf, sendBuf, sendLen + 1);
            txLen = sendLen;
            txIdx = 0;
            PIE1bits.SSPIE = 1;
